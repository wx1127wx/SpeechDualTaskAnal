...
**眨眼相关**
- `pupil_blink.csv`：眨眼相关指标
- `blink_event_detail.csv`：眨眼事件详细表
- `blink_data_use_raw.csv`：用于后续匹配与分析的眨眼整合表

**瞳孔 / HiPA 指标**
- `pupil_hipa.csv`：瞳孔 HiPA指标
- `hipa_all.csv`：HiPA 汇总表

**时间窗/任务匹配**
- `data_match30.csv`：将眼动指标与实验任务时间窗口对齐的匹配表

**被试/任务信息**
- `subinfo_speech.csv`：被试信息与任务条件信息
...
### 输出结果
- **眼动整合数据**：包含眨眼指标、瞳孔/HiPA 指标、时间窗匹配信息及被试/任务信息
...
3. `proc.default_subid` 用于默认被试编号/批处理配置（批处理多个被试）
...
**运行**
1. 运行 `3dttest_autocmd.py` 自动生成/整理 t-test 命令
2. 运行 `ttest_run.sh` 执行组水平 t-test
3. 运行 `batch_save_surfvol_figs.sh` 批量导出 surface/volume 可视化图
...
4. `3danova_autocmd.py` 用于自动生成/整理命令
